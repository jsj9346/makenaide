#!/bin/bash
"""
Makenaide EC2 자동 실행 스크립트
EC2 부팅 시 자동으로 Makenaide 파이프라인을 실행하고 완료 후 자동 종료

🎯 설치 방법:
1. 이 스크립트를 EC2에 업로드
2. /etc/systemd/system/makenaide-auto.service 생성
3. systemctl enable makenaide-auto 설정
