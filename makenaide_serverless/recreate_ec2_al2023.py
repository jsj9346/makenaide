...
def recreate_ec2():
    ...
    print('기존 인스턴스 종료 중...')
    try:
        ec2.terminate_instances(InstanceIds=['i-01082ade964089b29'])
        print('기존 인스턴스 종료 명령 완료')
    except Exception as e:
        print(f'기존 인스턴스 종료 오류 (무시 가능): {e}')
    
    # Amazon Linux 2023 AMI 찾기
    print('Amazon Linux 2023 AMI 찾는 중...')
    ...
    print(f'최신 Amazon Linux 2023 AMI: {ami_id} ({latest_ami["Name"]})')
...
echo "Python 설치 완료" > /home/ec2-user/setup_complete.txt
"""
    ...
    response = ec2.run_instances(
        ...
        TagSpecifications=[{
            ...
            'Tags': [{'Key': 'Name', 'Value': 'makenaide-ec2-al2023'}]
        }],
        UserData=user_data
    )
