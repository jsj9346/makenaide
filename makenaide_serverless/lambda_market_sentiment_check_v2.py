#!/usr/bin/env python3
"""
📊 Market Sentiment Check Lambda V2
...
# timezone_market_analyzer 모듈을 Lambda 레이어에서 불러오기
...
class MarketSentimentAnalyzerV2:
    """시장 상황 분석 및 리소스 제어 클래스 (시간대 분석 통합)"""
    ...
    def __init__(self):
        ...
        # 시간대 분석기 초기화 (임베디드 버전)
        self.timezone_analyzer = None
        try:
            from timezone_market_analyzer import TimezoneMarketAnalyzer
            self.timezone_analyzer = TimezoneMarketAnalyzer()
            logger.info("시간대 분석기 로드 성공")
        except Exception as e:
            logger.warning(f"시간대 분석기 로드 실패: {e}. 기본 모드로 실행.")
    ...
    def calculate_market_sentiment(self, btc_data: Dict, timezone_analysis: Optional[Dict] = None) -> Dict:
        """시장 상황 점수 계산 (시간대 분석 통합)"""
        try:
            sentiment_score = 0
            factors = []
            ...
            # 5. 시간대별 글로벌 활성도 보정 (추가 로직)
            if timezone_analysis:
                global_activity = timezone_analysis.get('global_activity_score', 50)
                market_condition_tz = timezone_analysis.get('market_condition', 'MODERATE')
                
                # 글로벌 활성도에 따른 점수 보정 (-10 ~ +10점)
                if global_activity >= 80:
                    sentiment_score += 10
                    factors.append(f"🌏 글로벌 활성도: 매우 높음 ({global_activity:.1f}%)")
                elif global_activity >= 60:
                    sentiment_score += 5
                    factors.append(f"🌏 글로벌 활성도: 높음 ({global_activity:.1f}%)")
                elif global_activity < 40:
                    sentiment_score -= 10
                    factors.append(f"🌏 글로벌 활성도: 낮음 ({global_activity:.1f}%)")
                else:
                    factors.append(f"🌏 글로벌 활성도: 보통 ({global_activity:.1f}%)")
                
                # 주도 지역 정보 추가
                dominant_regions = timezone_analysis.get('dominant_regions', [])
                if dominant_regions:
                    region_info = dominant_regions[0]
                    factors.append(f"🌐 주도 지역: {region_info['timezone'].split('/')[-1]}")
            ...
            return {
                ...
                'timezone_analysis': timezone_analysis  # 시간대 분석 결과 포함
            }
...
def lambda_handler(event, context):
    """Lambda 핸들러"""
    try:
        logger.info("=== Market Sentiment Check V2 시작 ===")
        ...
        current_kst_hour = current_kst.hour
        logger.info(f"현재 KST 시간: {current_kst_hour}시")
        
        # 시간대별 글로벌 시장 분석
        timezone_analysis = None
        if analyzer.timezone_analyzer:
            try:
                timezone_analysis = analyzer.timezone_analyzer.generate_comprehensive_analysis(current_kst_hour)
                logger.info(f"시간대 분석 완료: 글로벌 활성도 {timezone_analysis['global_activity_score']}%")
            except Exception as e:
                logger.warning(f"시간대 분석 실패: {e}")
        ...
        # 시장 상황 분석 (시간대 분석 결과 포함)
        sentiment = analyzer.calculate_market_sentiment(btc_data, timezone_analysis)
        if not sentiment:
...
"""
            ...
            # 시간대 분석 정보 추가
            if timezone_analysis:
                trading_style = timezone_analysis.get('trading_style', {})
                strategy_adj = timezone_analysis.get('strategy_adjustments', {})
                recommendations = timezone_analysis.get('recommendations', [])
                
                message += f"""
🌏 글로벌 시장 분석:
• 거래 스타일: {trading_style.get('primary_style', 'N/A')}
• 포지션 크기: {strategy_adj.get('position_size_modifier', 1.0) * 100:.0f}%
• 손절선: {strategy_adj.get('risk_parameters', {}).get('stop_loss_pct', 0.08) * 100:.0f}%

💡 권장사항:
{chr(10).join(recommendations[:3])}
"""
...
"""
        ...
        logger.info(f"=== Market Sentiment Check V2 완료 ===")
        logger.info(f"결과: {response}")
        
        return response
        
    except Exception as e:
        logger.error(f"Market Sentiment Check V2 실행 실패: {e}")
        return {
            'statusCode': 500,
            'body': json.dumps({
                'error': str(e),
                'message': 'Market Sentiment Check V2 실행 중 오류 발생'
            })
        }