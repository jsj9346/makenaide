...
def main():
    ...
    # 종합 검증 실행
    summary = validator.run_comprehensive_validation(days=7)
    ...
    # JSON 요약 저장
    json_filename = f"validation_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(json_filename, 'w', encoding='utf-8') as f:
        json.dump(summary, f, indent=2, ensure_ascii=False, default=str)
    
    print(f"\n📁 상세 보고서: {filename}")
    print(f"📁 JSON 요약: {json_filename}")
