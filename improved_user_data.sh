#!/bin/bash
#
# EC2 User Data 스크립트 - Makenaide 자동 실행 (개선 버전)
# EC2 인스턴스 시작 시 자동으로 Makenaide 파이프라인 실행
#
# 🎯 개선 사항:
# 1. 시스템 전역 환경변수 설정 (/etc/environment)
# 2. 사용자별 환경변수 설정 (.bashrc)
# 3. SSH 세션에서도 환경변수 접근 가능
# 4. 환경변수 영구 보존 (임시 파일 의존성 제거)
#
# 🎯 기능:
...
{
    echo "=================================================="
    echo "🚀 Makenaide EC2 자동 실행 시작 (개선 버전)"
    echo "=================================================="
    echo "시작 시간: $(date)"
...
# 6. 개선된 환경 변수 설정
log_info "🔧 환경 변수 설정 (시스템 전역 + 사용자별)"

# 6-1. 시스템 전역 환경변수 설정 (/etc/environment)
log_info "📝 시스템 전역 환경변수 설정 (/etc/environment)"
...
# 6-2. ec2-user 환경변수 설정 (.bashrc)
log_info "📝 ec2-user 환경변수 설정 (.bashrc)"
...
# ec2-user로 실행 (환경변수는 .bashrc에서 자동 로드됨)
sudo -u ec2-user bash << 'MAKENAIDE_EXECUTION'
set -e

cd /home/ec2-user/makenaide

# .bashrc 환경변수 로드 (SSH 세션과 동일한 환경)
...
{
    echo "=================================================="
    echo "🏁 Makenaide EC2 자동 실행 완료 (개선 버전)"
    echo "=================================================="
    echo "완료 시간: $(date)"
...
# 11. 정리 작업
log_info "🧹 임시 파일 정리"
# 주의: 환경변수는 영구 설정되었으므로 임시 파일만 정리
rm -f /tmp/environment_clean /tmp/bashrc_clean

if [ $EXECUTION_RESULT -eq 0 ]; then
    log_info "✅ User Data 스크립트 성공적으로 완료 (개선 버전)"
else
